...
var postcssCalcAstParser = require('postcss-calc-ast-parser');
...
function parseAndReduce(expr) {
    ...
    const calcParsed = postcssCalcAstParser.parse(unitlessExpr, { allowInlineCommnets: false });
    // Attempt to reduce the math expression
    const reduced = postcssCalcAstParser.reduceExpression(calcParsed);
    let unit;
